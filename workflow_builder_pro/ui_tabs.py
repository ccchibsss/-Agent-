        ...
        if selected_filename:
            ...
            if st.button("🚀 Применить", type="primary"):
                with st.spinner("Обработка..."):
                    ...
                    # Сохранение
                    save_path = st.session_state.image_manager.IMAGES_DIR / output_filename
...
Подробная документация доступна в папке `docs/` проекта.
...
def render_all_tabs():
    """Рендерит все вкладки - вызывается из app.py"""
    # Этот импорт здесь чтобы избежать циклических импортов
    from ai_agent import AgentManager
    ...
    # Здесь код вкладок будет вызван из main()
    pass
...
def render_analytics_tab_full(agent_manager: AgentManager):
    """Полная версия аналитики"""
    current_agent = agent_manager.get_current_agent()
    if not current_agent:
        st.warning("⚠️ Выберите агента")
        return
    ...
    with col1:
        st.metric("Всего диалогов", current_agent.stats['total_conversations'])
    with col2:
        st.metric("Обучающих примеров", len(current_agent.training_examples))
    with col3:
        st.metric("Фактов в памяти", len(current_agent.memory))
    
    if current_agent.conversation_history:
        st.subheader("Последние диалоги")
        for conv in current_agent.conversation_history[-5:]:
            with st.expander(f"📅 {conv['timestamp'][:16]}"):
