...
def render_sidebar() -> str:
    """Рендерит боковую панель с API ключом и расширенным управлением агентами"""
    ...
    for agent in agent_manager.agents.values():
        ...
        st.markdown(f"""
        <div class="{card_class}" style="margin-bottom: 10px; padding: 12px; border-radius: 12px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <strong>{agent.name}</strong><br>
                    <small style="color: #666;">{agent.role[:40]}</small>
                </div>
                <div>
                    <span style="background: #6974dc20; padding: 4px 8px; border-radius: 20px; font-size: 0.7rem;">обучений: {agent.stats['total_trainings']}</span>
                </div>
            </div>
            <div class="agent-stats" style="margin-top: 8px; font-size: 0.75rem;">
                📚 {agent.stats['total_trainings']} примеров | 💬 {agent.stats['total_conversations']} диалогов | 🧠 {len(agent.memory)} фактов
            </div>
        </div>
        ...
        with col1:
            if st.button(f"📋 Выбрать", key=f"select_{agent.id}", use_container_width=True):
                ...
                st.rerun()
        with col2:
            export_json = agent_manager.export_agent(agent.id)
            st.download_button(
                label="📤", data=export_json, file_name=f"{agent.name}.json", mime="application/json",
                key=f"export_{agent.id}", help="Экспортировать агента в JSON"
            )
        with col3:
    ...
    with st.expander("📂 ИМПОРТ АГЕНТА", expanded=False):
        uploaded_json = st.file_uploader("Выберите JSON файл агента", type=["json"], key="import_agent_json")
        if uploaded_json:
...
def render_chat_tab(agent_manager, api_key):
    ...
    user_input = st.text_area("✏️ Напишите сообщение...", height=80, key="chat_input",
                              placeholder="Введите ваш вопрос...", label_visibility="collapsed")
...
def render_analytics_tab(agent_manager):
    ...
    if current_agent.training_examples:
        ...
        df = pd.DataFrame([{'Дата': ex['timestamp'][:10], 'Пример': i+1} for i, ex in enumerate(current_agent.training_examples)])
        if not df.empty:
    ...
    with col1:
        ...
        for name, btype in blocks:
            if st.button(f"{name}", key=f"add_{btype}", use_container_width=True):
                st.session_state.workflow.append({"id": len(st.session_state.workflow), "name": name,
        ...
        for agent in agent_manager.agents.values():
            if st.button(f"🧠 {agent.name}", key=f"wf_agent_{agent.id}", use_container_width=True):
                st.session_state.workflow.append({"id": len(st.session_state.workflow), "name": f"Агент: {agent.name}",
...
""", unsafe_allow_html=True)
            ...
            with st.expander(f"⚙️ {block.get('name')}"):
                ...
                if node_type in [NodeType.EMAIL.value, NodeType.TELEGRAM.value, "sms", NodeType.DEEPSEEK_AI.value, NodeType.HTTP_POST.value]:
                    cfg['condition'] = st.text_area("🔀 Условие выполнения (опционально, на русском)", cfg.get('condition',''),
                                                    height=60, key=f"cond_node_{i}", help="Пример: если статус равно 'успех'")
                if node_type == NodeType.DEEPSEEK_AI.value:
                    ...
                    cfg['bot_token'] = st.text_input("Bot Token (опционально)", cfg.get('bot_token',''), type="password", key=f"tg_token_{i}")
                elif node_type == "sms":
        ...
        if st.button("🚀 ЗАПУСТИТЬ WORKFLOW", type="primary", use_container_width=True):
            ...
            executor = WorkflowExecutor(st.session_state.workflow, api_key, agent_manager, st.session_state.table_manager)
            ...
            if result['success']:
                ...
                with st.expander("📋 Детали выполнения", expanded=True):
                    for res in result['results']:
                        ...
                        st.json({'node': res['node'], 'type': res['type'], 'result': clean_result})
...
def render_tables_tab(api_key):
    ...
    with col1:
        uploaded_file = st.file_uploader("📂 Выберите файл", type=['xlsx','xls','csv'], key="ai_upload_excel")
        file_type = "excel"
        if uploaded_file and uploaded_file.name.endswith('.csv'):
            file_type = "csv"
    with col2:
        ai_instruction = st.text_area("✏️ Инструкция для ИИ", height=120,
        ...
        if not api_key:
            ...
            with st.spinner("🧠 ИИ анализирует и применяет изменения..."):
                result = st.session_state.table_manager.ai_edit_excel_file(
                    input_file=uploaded_file, instruction=ai_instruction, api_key=api_key,
                    sheet_name=0, file_type=file_type
                )
            if result['success']:
                ...
                new_name = f"fixed_{uploaded_file.name.rsplit('.',1)[0]}.{ext}"
                st.download_button(label="📥 Скачать обработанный файл", data=result['output_bytes'],
                                   file_name=new_name, mime=mime, use_container_width=True)
            else:
    ...
    with col2:
        if st.session_state.current_df is not None:
            ...
            edited_df = st.data_editor(st.session_state.current_df, num_rows="dynamic", use_container_width=True,
                                       key=f"editor_{st.session_state.editing_table_id}", hide_index=True)
...
def render_images_tab(api_key):
    ...
    supported_formats = getattr(CONFIG, 'SUPPORTED_IMAGE_FORMATS', ['jpg','jpeg','png','webp','bmp','gif'])
    uploaded_files = st.file_uploader("Выберите изображения", type=list(supported_formats),
        ...
        if selected_filename:
            ...
            if st.button("🚀 Применить", type="primary"):
                with st.spinner("Обработка..."):
                    processed = image_manager._apply_operation(original, ImageEditOperation(operation), {'api_key': api_key})
...
- 📥 Массовая загрузка
- 🎨 Удаление фона (rembg)
- 💧 Удаление водяных знаков через ИИ
- 💬 Добавление текста
---
## 🗂 Редактирование таблиц
- ✏️ Полноценный редактор
- 💾 Автосохранение
- 📥 Экспорт в Excel
- 🤖 ИИ-трансформация
---
