...
class ImageManager:
    ...
    def remove_watermark_ai(self, image: Image.Image, api_key: str = None) -> Dict[str, Any]:
        """Удаление водяного знака через ИИ (DeepSeek API)"""
...
Отвечай ТОЛЬКО валидным JSON."""
            ...
            return {
                'success': True,
                'image': processed_image,
                'analysis': 'Использован базовый метод',
                'method': 'basic'
            }
    ...
    def _apply_ai_watermark_removal(self, image: Image.Image, ai_result: Dict) -> Image.Image:
        """Применяет ИИ-рекомендации для удаления водяного знака"""
        ...
        if method == 'inpainting':
            ...
            # Определяем область (центральная часть как предположительное место водяного знака)
    ...
    def remove_watermark_basic(self, image: Image.Image) -> Image.Image:
        """Базовое удаление водяного знака"""
        img_array = np.array(image)
