...
class TableManager:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self._history: Dict[str, List[pd.DataFrame]] = {}  # история изменений для таблиц
    ...
    def read_google_sheets(
        ...
        """Читает данные из Google Sheets (через экспорт CSV)"""
        if '/d/' in url:
    ...
    def ai_analyze_dataframe(self, df: pd.DataFrame, instruction: str, api_key: str) -> Dict[str, Any]:
        """Анализирует DataFrame через ИИ и возвращает план трансформаций"""
        if not api_key:
            return {'error': 'API ключ не указан'}
        
        # Подготовка сводки DataFrame
        df_summary = {
            'shape': df.shape,
            'columns': list(df.columns),
...
"""
    ...
    def undo_last(self, table_id: str) -> Optional[pd.DataFrame]:
        ...
        if table_id in self._history and len(self._history[table_id]) > 1:
            self._history[table_id].pop()  # удаляем текущее состояние
            return self._history[table_id][-1].copy()
        return None
    
    # ================== НОВЫЙ МЕТОД: ИИ-РЕДАКТИРОВАНИЕ EXCEL/CSV ==================
    @handle_errors(default_return=None)
    def ai_edit_excel_file(
        ...
        file_type: str = "excel"  # "excel" или "csv"
    ) -> Dict[str, Any]:
        """
        Редактирует таблицу (Excel или CSV) по инструкции на русском языке.
        Возвращает результат с изменённым DataFrame и байтами для скачивания.
        """
        result = {
            'success': False,
            'df': None,
        ...
        # Сохраняем исходное состояние в историю
        ...
        for trans in ai_res.get('transformations', []):
            ...
            if not code:
                continue
            try:
                self.save_to_history(table_id, current_df)  # сохраняем перед применением
                ...
                if undo_df is not None:
                    current_df = undo_df
                    result['message'] += f" Откачено последнее изменение из-за ошибки: {e}"
                else:
                    return result
        ...
        # Подготовка выходного байтового потока
        output = BytesIO()
        if file_type == "csv":
