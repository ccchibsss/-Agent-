...
# Импортируем утилиты из проекта (предполагается, что они есть в вашей структуре)
...
class TableManager:
    ...
    def read_google_sheets(
        ...
        """Читает данные из Google Sheets (публичная таблица, без API)"""
        if '/d/' in url:
...
"""
    ...
    def execute_transformation(
        ...
        """Выполняет код трансформации над DataFrame (безопасно)"""
        # Безопасность: проверяем опасные операции
    ...
    def update_google_sheets(self, url: str, df: pd.DataFrame, sheet_name: Optional[str] = None) -> bool:
        """
        Запись в Google Sheets требует API и сервисного аккаунта.
        Для работы без API эта функция возвращает False с пояснением.
        """
