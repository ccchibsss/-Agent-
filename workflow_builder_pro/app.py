...
def initialize_session_state():
    defaults = {
        'agent_manager': None, 'workflow': [], 'agent_messages': [],
        'history': [], 'analytics': {'total_executions':0,'successful_executions':0,'failed_executions':0},
        'voice_show_upload': False, 'table_manager': None, 'current_df': None,
        'excel_loaded': False, 'data_loaded': False, 'saved_tables': {},
        'table_edit_mode': False, 'editing_table_id': None, 'image_manager': None,
        'uploaded_images': {}, 'processed_images': {}, 'image_batch_progress': 0,
    }
    ...
    if not st.session_state.data_loaded:
        ...
        if saved_workflow: st.session_state.workflow = saved_workflow
        saved_messages = load_messages_auto()
        if saved_messages: st.session_state.agent_messages = saved_messages
        saved_history = load_history_auto()
        if saved_history: st.session_state.history = saved_history
        ...
        if saved_tables: st.session_state.saved_tables = saved_tables
...
def main():
    ...
    # Автосохранение
    if st.session_state.workflow: save_workflow_auto(st.session_state.workflow)
    if st.session_state.agent_messages: save_messages_auto(st.session_state.agent_messages)
    if st.session_state.saved_tables: save_tables_auto(st.session_state.saved_tables)
    ...
    with st.sidebar:
        ...
        api_key = st.text_input("🔑 DeepSeek API Ключ", type="password", help="platform.deepseek.com", key="api_key_main")
        st.markdown("---")
        ...
        with st.expander("➕ СОЗДАТЬ АГЕНТА", expanded=False):
            name = st.text_input("Имя", key="new_agent_name")
            role = st.text_input("Роль", key="new_agent_role")
        ...
        with st.expander("🔄 ЭКСПОРТ/ИМПОРТ"):
            ...
            if imp:
                if agent_manager.import_agent(imp.read().decode('utf-8')):
                    st.success("✅ Импортирован!")
                    st.rerun()
        st.markdown("---")
        with st.expander("🗑️ Управление данными"):
            if st.button("Сбросить workflow"): st.session_state.workflow = []; st.rerun()
            if st.button("Очистить чат"): st.session_state.agent_messages = []; st.rerun()
            if st.button("Сбросить ВСЁ", type="secondary"):
                ...
                if IMAGES_DIR.exists(): shutil.rmtree(IMAGES_DIR); IMAGES_DIR.mkdir(exist_ok=True)
    ...
    tabs = st.tabs(["💬 Диалог","📚 Обучение","🧠 Память","📊 Аналитика",
                    "🤖 Workflow","🔀 Условия","🗂 Таблицы+ИИ","🖼️ Изображения","📖 Справка"])
    with tabs[0]: render_chat_tab(agent_manager, api_key)
    with tabs[1]: render_training_tab(agent_manager)
    with tabs[2]: render_memory_tab(agent_manager)
    with tabs[3]: render_analytics_tab(agent_manager)
    with tabs[4]: render_workflow_tab(agent_manager, api_key)
    with tabs[5]: render_conditions_tab()
    with tabs[6]: render_tables_tab(api_key)
    with tabs[7]: render_images_tab(api_key)
    with tabs[8]: render_help_tab()
